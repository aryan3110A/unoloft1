...
export default function HeroSection({ mode }: HeroSectionProps) {
  ...
  const [sources, setSources] = useState(() =>
    HERO_MOSAIC_CARDS.map((card) => card.sources.all),
  );
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setSources(
        HERO_MOSAIC_CARDS.map((card) => card.sources[mode] ?? card.sources.all),
      );
      setFading(false);
    }, 280);
        ...
        <div className="hero-r">
          {HERO_MOSAIC_CARDS.map((card, index) => (
            <div className={`mc ${card.cardClass}`} key={card.id}>
              <img
                id={card.id}
                data-all={card.sources.all}
                data-boys={card.sources.boys}
                data-girls={card.sources.girls}
                src={sources[index]}
                alt={card.alt}
